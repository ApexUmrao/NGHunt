...
public class C_Event implements IEventHandler{
	...
	public LapsModifyResponse dispatchEvent(CoreEvent paramCoreEvent) throws Exception {
		LapsModifyLogger.logMe(LapsModifyLogger.LOG_LEVEL_INFO,"Inside C_Event");
...
//				LapsConfigurations.getInstance().Password);
		...
		if (isChannelExist==true){
			resp = triggerEvent();
			LapsModifyLogger.logMe(LapsModifyLogger.LOG_LEVEL_INFO,"C_Event response from C_"+channelName+": "
					+resp.toString());
			return resp;
		}
		else{
