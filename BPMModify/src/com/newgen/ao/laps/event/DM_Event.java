...
public class DM_Event implements IEventHandler{
	...
	public LapsModifyResponse dispatchEvent(CoreEvent paramCoreEvent) throws Exception {
		LapsModifyLogger.logMe(LapsModifyLogger.LOG_LEVEL_INFO,"Inside DM_Event >>");
		...
		if (isChannelExist==true){
			resp = triggerEvent();
			LapsModifyLogger.logMe(LapsModifyLogger.LOG_LEVEL_INFO,"DM_Event response from "+sourcingChannel+": "
					+resp.toString());
			return resp;
		} else {
