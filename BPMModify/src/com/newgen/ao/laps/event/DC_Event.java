...
public class DC_Event implements IEventHandler{
	...
	public LapsModifyResponse dispatchEvent(CoreEvent paramCoreEvent) throws Exception {
		LapsModifyLogger.logMe(LapsModifyLogger.LOG_LEVEL_INFO,"Inside DC_Event >>");
		...
		if (isChannelExist==true){
			resp = triggerEvent();
			LapsModifyLogger.logMe(LapsModifyLogger.LOG_LEVEL_INFO,"DC_Event response from "+sourcingChannel+": "
					+resp.toString());
			return resp;
		} else {
