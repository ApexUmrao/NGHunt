...
public class CheckDuplicatePassport {
	public static boolean isDuplicate(String passportNumber, String nationality, String DOB, String cabinetname, CBGSingleHookResponse responseObj){
		...
		try {
			...
			cstmt.setString(4, passportNumber);//PASSPORTNo
			cstmt.setString(5, nationality);//Nationality
			cstmt.setString(6, DOB);//DOB
			...
			con = null;
			if(isDeDupe){
				String outputXML = APCallCreateXML.APSelect("SELECT STAGE_NAME, LEAD_REF_NO, WI_NAME  FROM EXT_CBG_CUST_ONBOARDING WHERE PASSPORT_NUMBER = '"+ passportNumber +"'");
