...
public class SaveEIDADetails extends DSCOPServiceHandler
{
	...
	public String saveEIDADetailsStatus(String sInputXML) throws RemoteException
	{
		...
		LogGEN.writeTrace("CBG_Log", "Fuction called saveEIDADetailsStatus");
		LogGEN.writeTrace("CBG_Log", "input xml saveEIDADetailsStatus: ");
		...
		try
		{
			...
			String EIDAValidated=xmlDataParser.getValueOf("EIDAValidated");
			...
			String customerInformation = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
					...
					"<EIDAValidated>"+EIDAValidated+"</EIDAValidated>"+
					"</EIDADetails>";
			...
			LogGEN.writeTrace("CBG_Log", "customerInformation---- "+customerInformation);
			xmlInput=stub.getinputXML(reqMessage);
			//			LogGEN.writeTrace("CBG_Log", "SaveEIDADetails input xml "+xmlInput);
			resMessage = stub.modCustMDMPubInfo_Oper(reqMessage);	
			sOrg_Output=stub.resEidaMsg;//Added By Harish For inserting original mssg
			//			LogGEN.writeTrace("CBG_Log", "SaveEIDADetails output xml "+sOrg_Output);
