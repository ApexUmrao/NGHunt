...
public class AddLead extends DSCOPServiceHandler  {
	...
	public String addLeadResponse(String sInputXML) 
	{
		...
		try
		{
			...
			String productDetails = xmlDataParser.getValueOf("productDetails");
			String entityNamePD = "";
			EntityRecord_type1[] entrityRecordPD = null;
			LeadServiceModule_type1 lsModulePD = null;
			if(!productDetails.isEmpty()){
				XMLParser xmlDataPD = new XMLParser();
				xmlDataPD.setInputXML(productDetails);
				entityNamePD = xmlDataPD.getValueOf("entityNamePD");
				int noOfEntityRecords = xmlDataPD.getNoOfFields("entityRecordPD");
				entrityRecordPD = new EntityRecord_type1[noOfEntityRecords];
				for (int noofTag = 0; noofTag < noOfEntityRecords; ++noofTag) {
					XMLParser innerXMLPD = new XMLParser();
					if(noofTag==0) {
						innerXMLPD.setInputXML(xmlDataPD.getFirstValueOf("entityRecordPD"));
						int noOfRecordsPD = innerXMLPD.getNoOfFields("attributeNameValPairPD");
						AttributeNameValPair_type1[] attrNameValPairPD = new AttributeNameValPair_type1[noOfRecordsPD];
						for (int noofTagInner = 0; noofTagInner < noOfRecordsPD; ++noofTagInner) {
							XMLParser innerParser = new XMLParser();
							if(noofTagInner==0)
								innerParser.setInputXML(innerXMLPD.getFirstValueOf("attributeNameValPairPD"));
							else
								innerParser.setInputXML(innerXMLPD.getNextValueOf("attributeNameValPairPD"));
							...
							attrNameValPairPD[noofTagInner]=nvPair;
						}
						
						EntityRecord_type1 eRecordPD = new EntityRecord_type1();
						eRecordPD.setAttributeNameValPair(attrNameValPairPD);
						entrityRecordPD[noofTag]=eRecordPD;
						
					} else {
						innerXMLPD.setInputXML(xmlDataPD.getNextValueOf("entityRecordPD"));
						int noOfRecordsPD = innerXMLPD.getNoOfFields("attributeNameValPairPD");
						AttributeNameValPair_type1[] attrNameValPairPD = new AttributeNameValPair_type1[noOfRecordsPD];
						for (int noofTagInner = 0; noofTagInner < noOfRecordsPD; ++noofTagInner) {
							XMLParser innerParser = new XMLParser();
			...
			if(!productDetails.isEmpty()) {
				lsModuleSize = 2;
				lsModulePD = new LeadServiceModule_type1();
				lsModulePD.setEntityName(entityNamePD);
				lsModulePD.setEntityRecord(entrityRecordPD);
			}
			LeadServiceModule_type1[] leadServiceModule = new LeadServiceModule_type1[lsModuleSize];
			...
			if(lsModulePD != null && !productDetails.isEmpty()){
				leadServiceModule[1]=lsModulePD;
			}
