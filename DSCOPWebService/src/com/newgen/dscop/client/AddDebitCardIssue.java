...
import com.newgen.dscop.stub.AddDebitCardIssueStub;
...
import com.newgen.dscop.stub.AddDebitCardIssueStub.HeaderType;


public class AddDebitCardIssue extends DSCOPServiceHandler {
	...
	public String AddDebitCards(String sInputXML)
	{
		...
		try
		{
			...
			ArrayList<String>  wsConfig = WebServiceConfig.getInstance().getCBGWSConfig().get("Add_Debit_Card_Issuence");
			LogGEN.writeTrace("CBG_Log", "Add_Debit_Card_Issuence WebServiceConfig Map : "  +wsConfig.toString());
			...
			header_req.setServiceName("AddDebitCardIssue"); 
			...
			add_req.setWorkIdRefNbr(sWorkIdRefNbr);
			add_request.setHeader(header_req);
			add_request.setAddDebitCardIssueReq(add_req);
			LogGEN.writeTrace("CBG_Log", "CBG AddDebitCardIssue add_request---");
			xmlInput = stub.getinputXML(add_request);
			LogGEN.writeTrace("CBG_Log", "CBG AddDebitCardIssue xmlInput---");
			...
			LogGEN.writeTrace("CBG_Log", "CBG AddDebitCardIssue Return Code---"+sReturnCode);
			LogGEN.writeTrace("CBG_Log", "CBG AddDebitCardIssue Error Detail---"+sErrorDetail);
			LogGEN.writeTrace("CBG_Log", "CBG AddDebitCardIssue Error Description---"+sErrorDesc);
