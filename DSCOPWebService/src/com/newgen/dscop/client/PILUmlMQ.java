...
public class PILUmlMQ extends DSCOPServiceHandler {
	String xmlInput = "";
	String sOrgRes  = "";
	...
	public String pilUMLMQPut(String inputXML) {

		LogGEN.writeTrace("CBG_Log", "Function Called : pilUMLMQPut");
		...
		String payload = xmlDataParser.getValueOf("PAYLOAD");
		...
		LogGEN.writeTrace("CBG_Log", "pilUMLMQPut MQ DBUrl : " + this.dburl);
		LogGEN.writeTrace("CBG_Log", "pilUMLMQPut MQ DBUser: " + this.dbuser);
		LogGEN.writeTrace("CBG_Log", "pilUMLMQPut MQ DBPass: " + this.dbpass);
		...
		try {
			...
			inputXml.append("<PAYLOAD type=\"PIL_UML\">")
			.append(payload)
			.append("</PAYLOAD>");
			...
			sOrgRes = mq.MQPutGetMessage(xmlInput,"PILUmlMQ");
			...
			output = "<?xml version=\"1.0\" encoding=\"utf-8\"?><Option>pilUMLMQPut</Option><Output><returnCode>" + returnCode + "</returnCode><errorDescription>" + errorDesc + "</errorDescription></Output>";
			...
			String Query="insert into USR_0_DSCOP_WS_AUDIT(winame,userid,request_type,request,requestdatetime,response,responsedatetime,status) values('"+winame+"',( select username from pdbuser a, pdbconnection b   where a.userindex=b.userindex "+
					...
					"?,sysdate,'')";
			LogGEN.writeTrace("CBG_Log"," pilUMLMQPut Application Detail Report NI Query : finally :"+Query);
			try {
				con.executeClob("jdbc:oracle:thin:@"+dburl,dbuser,dbpass, Query, this.xmlInput.replaceAll("'", "''"),"");
			} catch (Exception e) {
				LogGEN.writeTrace("CBG_Log", "Exception pilUMLMQPut sFinalQuery : " + e.getMessage());
			}
