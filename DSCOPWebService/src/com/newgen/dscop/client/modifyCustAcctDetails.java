...
import com.newgen.dscop.stub.ModCustAcctDetailsStub.AccountStatusUpdate_type1;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.CustomerRelationUpdate_type0;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.CustomerRelationUpdate_type1;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.HeaderType;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.MemoDetailsUpdate_type0;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.MemoDetailsUpdate_type1;
...
import com.newgen.dscop.stub.ModCustAcctDetailsStub.ModCustAcctDetailsResChoice_type0;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.ModCustAcctDetailsResMsg;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.ModCustAcctDetailsRes_type0;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.ServiceChargesUpdate_type0;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.ServiceChargesUpdate_type1;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.StandingInstructionUpdate_type0;
import com.newgen.dscop.stub.ModCustAcctDetailsStub.StandingInstructionUpdate_type1;
import com.newgen.dscop.stub.modifyCustSig;
import com.newgen.dscop.stub.modifyCustSig.CustomerSignatureUpdate_type1;
public class modifyCustAcctDetails extends DSCOPServiceHandler 
{
	...
	{
		LogGEN.writeTrace("CBG_Log", "Fuction called Modify_Account");
		LogGEN.writeTrace("CBG_Log", "modify_account sInputXML---"+sInputXML);
		...
		try
		{
			...
			ArrayList<String>  wsConfig = WebServiceConfig.getInstance().getCBGWSConfig().get("Modify_Account");
			LogGEN.writeTrace("CBG_Log", "Modify_Account WebServiceConfig Map : "  +wsConfig.toString());
			...
			LogGEN.writeTrace("CBG_Log", "Modify_Account WSDL PATH: "+sWSDLPath);
			LogGEN.writeTrace("CBG_Log", "Modify_Account CABINET: "+sCabinet);
			LogGEN.writeTrace("CBG_Log", "Modify_Account USER: "+sUser);
			LogGEN.writeTrace("CBG_Log", "Modify_Account PASSWORD: "+sPassword);
			LogGEN.writeTrace("CBG_Log", "Modify_Account LOGIN_REQ: "+sLoginReq);
			LogGEN.writeTrace("CBG_Log", "Modify_Account TIME_OUT: "+lTimeOut);
			...
			LogGEN.writeTrace("CBG_Log", "custAccountNumber---"+xmlDataParser.getValueOf("custAccountNumber"));
			...
			if(call_type.equalsIgnoreCase("accountStatusUpdate"))
			{
				...
				LogGEN.writeTrace("CBG_Log","No_Credit:"+No_Credit);
				LogGEN.writeTrace("CBG_Log","No_Debit:"+No_Debit);
				LogGEN.writeTrace("CBG_Log","Dormant:"+Dormant);
				LogGEN.writeTrace("CBG_Log","Frozen:"+Frozen);
			...
			if(call_type.equalsIgnoreCase("customerSignatureUpdate"))
			{
				//LogGEN.writeTrace("CBG_Log","inside signature upload");
			...
			LogGEN.writeTrace("CBG_Log","before calling function");
			if(!call_type.equalsIgnoreCase("customerSignatureUpdate"))
			{
			...
			LogGEN.writeTrace("CBG_Log", "All values set xmlInput modifyCustAcctDetails " + xmlInput);

			LogGEN.writeTrace("CBG_Log", "Mohit input:"+acc_stub.getinputXML(acc_Req_Msg));
			// System.out.println(acc_stub.getinputXML(acc_Req_Msg));

			LogGEN.writeTrace("CBG_Log","sdafasdfasdfsfsdf");
			if(call_type.equalsIgnoreCase("customerSignatureUpdate"))
			{
				try
				{
					LogGEN.writeTrace("CBG_Log","21312312313123123123131231233123");
					LogGEN.writeTrace("CBG_Log",acc_Rep_Msg_sig);
					...
					LogGEN.writeTrace("CBG_Log", call_type + " CBG " + sOutput);
				}
				catch(Exception ee)
				{
			...
			{
				LogGEN.writeTrace("CBG_Log","3453453453453453453");
				Header_Input= acc_Rep_Msg.getHeader();
				sReturnCode= Header_Input.getReturnCode();
				sErrorDetail=Header_Input.getErrorDetail();
				sErrorDesc = Header_Input.getErrorDescription();
				...
				{		
					...
					sOutput+="</Output>";
					LogGEN.writeTrace("CBG_Log", "modifycustAcctDtls Output XML--- "+sOutput);
				}
				else
				{
		...
		{
			LogGEN.writeTrace("CBG_Log", "modifycustAcctDtls Error in Web Serviice :"+e.toString());
			LogGEN.writeTrace("CBG_Log", "modifycustAcctDtls Error Trace in Web Serviice :"+e.getStackTrace());	
			sReturnCode="-1";
		...
		{
			LogGEN.writeTrace("CBG_Log","modifycustAcctDtls outputXML.trim().length() :"+sOutput.trim().length());				
			if(sOutput.trim().length()<1)
			{
				sOutput="<?xml version=\"1.0\" encoding=\"utf-8\"?><Option>Modify_Account</Option><Output><returnCode>"+sReturnCode+"</returnCode><errorDescription>"+sErrorDetail+"</errorDescription><td>Unable to modify account.</td></Output>";
			}

			LogGEN.writeTrace("CBG_Log","modifycustAcctDtls outputXML : finally :"+sOutput);
