...
public class CustomerPersonalDetailsDA extends DSCOPServiceHandler{
	...
	{
		LogGEN.writeTrace("CBG_Log", "Fuction called CustomerPersonalDetailsDA");
		...
		try
		{
			...
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA WebServiceConfig Map : "  +wsConfig.toString());
			...
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA WSDL PATH: "+sWSDLPath);
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA CABINET: "+sCabinet);
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA USER: "+sUser);
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA PASSWORD: "+sPassword);
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA LOGIN_REQ: "+sLoginReq);
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA TIME_OUT: "+lTimeOut);
			...
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA All Objects created");	
			...
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA All values set");
			...
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA Request: "+xmlInput);
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA Response: "+sOrg_Output);
		...
		{			
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA Error in Web Service :"+e.toString());
			LogGEN.writeTrace("CBG_Log", "CustomerPersonalDetailsDA Error Trace in Web Service :"+e);
		...
		{
			LogGEN.writeTrace("CBG_Log","CustomerPersonalDetailsDA outputXML.trim().length() :"+sOutput.trim().length());				
	...
	private HeaderType setHeaderDtls(String sDate,String ref_no,String senderId){
		...
		headerType.setUsecaseID("DormantAccount");
		headerType.setServiceName("InqCustPersonalDtl");
		headerType.setVersionNo("1.1");
