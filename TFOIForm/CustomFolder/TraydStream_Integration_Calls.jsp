		...
		try
		{
			...
			if(counter<=document.getElementById('CallTypeTable').rows.length)
			{
				...
				if(counter!=0 && counter<tbl.rows.length && tbl.rows.item(counter).cells.item(2).innerText!='Y ')
				{
					...
					var tbl=document.getElementById('CallTypeTable');	
					var currentrowCallOperation = tbl.rows.item(counter).cells.item(0).innerText;
					...
					if(callRequestType=="A")
					{	
						xmlobj.open("POST","TraydStream_Integration_Calls_Executor.jsp?WD_UID="+wd_uid+"&rnd="+rnd+"&WI_NAME="+ workitem +"&seq="+(counter)+"&CALL_OPERATION="+currentrowCallOperation,true);
						//alert('after');
					}  
					//alert('WORKING');
					xmlobj.send("sd");
...
//alert("CALLNAME: "+sCallName);
	...
	var tbl=document.getElementById('CallTypeTable');	
		if(callRequestType=="A")
		{
		xmlobj.open("POST","TraydStream_Integration_Calls_Executor.jsp?WD_UID="+wd_uid+"&rnd="+rnd+"&WI_NAME="+ workitem +"&seq="+(row_num)+"&CALL_OPERATION="+callOperation,true);
		}
		xmlobj.send("sd");	
...
<%
	try
	{
		...
		if(!sWIName.equals(""))
		{
	           sQuery= "select CALL_NAME,CALL_OPERATION, CALL_STATUS, STATUS, to_char(CALL_DT,'dd/mm/yyyy hh:mi:ss') CALL_DATE, ERROR_DESCRIPTION from TFO_DJ_INTEGRATION_CALLS WHERE WI_NAME =N'"+sWIName+"' AND CALL_STATUS = N'F'";
	...
	if(mandatoryfail==true)
		...
		if(callRequestType=='OpenLinkWIJSP'){
			window.parent.handleJSPResponsePC('OpenLinkWIJSP',val);	
				...
				window.parent.handleJSPResponse('TRAYDSTREAM_INTEGRATION_CALLS',val);	
			}
