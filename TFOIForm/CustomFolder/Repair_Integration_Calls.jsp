			...
			if(counter<=document.getElementById('CallTypeTable').rows.length)
			{
				...
				xmlobj.onreadystatechange=function()			
				{
					...
					if(xmlobj.readyState==4 && xmlobj.status==200)
					{
						...
						else
						{
							...
							//alert('resval[0] '+xmlobj.responseText.replace(/^\s+|\s+$/g,'').indexOf("0#") );
							if(xmlobj.responseText.replace(/^\s+|\s+$/g,'').indexOf("0#")>=0)
							{  
								//alert('2- xmlobj.responseText: '+xmlobj.responseText.replace(/^\s+|\s+$/g,''));
							...
							//	tbl.rows.item(counter).cells.item(3).innerHTML=resval[1];
								...
								//alert('call ' + tbl.rows.item(counter).cells.item(0).innerHTML);
								//alert('resval[0] : '+resval[0]);
								var resvalLink=	xmlobj.responseText.replace(/^\s+|\s+$/g,'').split('#');
								if(tbl.rows.item(counter).cells.item(0).innerHTML == 'LinkDocuments' && resvalLink[0] == '0'){
									//alert('resvalLink[3] : '+resvalLink[3]);
									ngAddDocOutput = resvalLink[3];
								}
							...
							//alert('after green');
								...
								if(callRequestType=='OpenLinkWIJSP'||callRequestType=='openRepairJSP'){
				...
				if(counter!=0 && counter<tbl.rows.length && tbl.rows.item(counter).cells.item(2).innerText!='Y ')
				{
				   ...
				   // alert('callRequestType : '+callRequestType);
					if(callRequestType=="openRepairJSP")
					{	
				     // alert(tbl.rows.item(counter).cells.item(0).innerText);
					  
					  var callName=tbl.rows.item(counter).cells.item(0).innerText;
					  var stageId=tbl.rows.item(counter).cells.item(5).innerText;
					 //alert(stageId);
					// alert('33');
					// alert(callName.includes('LinkWorkitem'));
					 if(callName.includes('LinkWorkitem')){
						 xmlobj.open("POST","Create_Link_Workitem.jsp?WD_UID="+wd_uid+"&rnd="+rnd+"&WI_NAME="+ workitem +"&seq="+(counter)+"&CALL_OPERATION="+"LinkWorkitem",true);
					 }else{
						 xmlobj.open("POST","Repair_Integration_Calls_Executor.jsp?WD_UID="+wd_uid+"&rnd="+rnd+"&WI_NAME="+ workitem +"&seq="+(counter)+"&CALL_NAME="+callName+"&STAGE_ID="+stageId,true);
					 }
...
<th style="background-color: #ba1b18;visibility:hidden" width="20%" valign="top" colspan=2 >
			<font color="#ffffff" face="Calibri" size="3"><b>Stage ID</b></font>
		</th>		
	...
	try
	{
		...
		if(!sWIName.equals(""))
		{
	          // sQuery= "select CALL_NAME,CALL_OPERATION, CALL_STATUS, STATUS, to_char(CALL_DT,'dd/mm/yyyy hh:mi:ss') CALL_DATE, ERROR_DESCRIPTION from //TFO_DJ_INTEGRATION_CALLS WHERE WI_NAME =N'"+sWIName+"' AND CALL_STATUS = N'N'";
			  		sQuery= "select  call_name,CALL_STATUS as CALL_OPERATION,CALL_STATUS,CALL_STATUS as STATUS, to_char(CALL_DT,'dd/mm/yyyy hh:mi:ss') CALL_DATE,ERROR_DESCRIPTION, ABS(STAGE_ID) as STAGE_ID from bpm_orchestration_status  x where wi_name='"+sWIName+"' and  call_status='N' and CALL_DT in (select min(CALL_DT) from   bpm_orchestration_status where wi_name='"+sWIName+"'  and  call_status='N' and  call_name=x.call_name )  union select CALL_NAME,CALL_OPERATION, CALL_STATUS, STATUS, to_char(CALL_DT,'dd/mm/yyyy hh:mi:ss') CALL_DATE, ERROR_DESCRIPTION, 1 as STAGE_ID  from TFO_DJ_INTEGRATION_CALLS where call_operation='Linked_Workitem_Creation' and  wi_name='"+sWIName+"'and call_status='N'";
		}
		...
		if(!sOutput.equals(""))
		{
			...
			{	int i=1;				
				while((sOutput.indexOf("<Record>")>-1))
				{
					...
					sStageId = sRecord.substring(sRecord.indexOf("<STAGE_ID>")+10,sRecord.indexOf("</STAGE_ID>"));
					...
					%>
						...
						<TD width="20%" style="FONT-FAMILY: Microsoft Sans Serif;FONT-WEIGHT: normal;font-size: 10pt;display:none"><%= sStageId%></TD>									
						</TR>
					<%
					i++;
...
function closeandReturn()
{
	...
	for(i=1;i<tbl.rows.length;i++)
	{
		...
		var callStatus=tbl.rows.item(i).cells.item(2).innerText;
		var callName=tbl.rows.item(i).cells.item(0).innerText;
		if(callName.includes('LinkWorkitem')){
					 wiNumber=tbl.rows.item(i).cells.item(4).innerText;
		...
		if(callRequestType=='openRepairJSP'){
			//alert('wiNumber : '+wiNumber);
			//alert('ngAddDocOutput : '+ngAddDocOutput);
			window.parent.handleJSPResponse('openRepairJSP',val+"#"+wiNumber+"#"+ngAddDocOutput);	
		}
