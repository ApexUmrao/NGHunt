...
public class CBGLogMe {
	...
	private static CBGLogMe instance;
	...
	public static CBGLogMe getInstance(){
		if(instance==null){
			instance = new CBGLogMe();
		}
		return instance;
	}
	...
	public void intitalizeLog(){
		try {
			...
			StringBuilder log4JPropertyFile = new StringBuilder().append("DSCOP_log4j.properties");

			System.out.println("Path of log4j file: " + log4JPropertyFile);
			InputStream stream = new FileInputStream(log4JPropertyFile.toString());
			if (stream == null) {
				System.out.println("CBG_log4j.properties not found");
			}
	...
	private String buildExceptionText(Throwable t) throws Exception {
		...
		t.printStackTrace();
		String toReturn = "\n\r" + t.toString() + "(" + t.getMessage() + ")";
		StackTraceElement[] trace = t.getStackTrace();
		for (StackTraceElement element : trace) {
			toReturn += "\n\r" + element.getClassName() + "."
	...
	private void log4j(int level, String logText) throws Exception {
		if (level == CBGLogMe.LOG_LEVEL_INFO) {
			log4j.info(logText);
			return;
		} 
		else if (level == CBGLogMe.LOG_LEVEL_ERROR) {
