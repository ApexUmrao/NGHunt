...
public class DeliveryMaker extends Common implements Constants,ConstantAlerts,
IFormServerEventHandler {

	public DeliveryMaker(IFormReference iFormReference) {
	...
	public String executeServerEvent(IFormReference formObject, String controlName, String eventType, String data) {
		...
		try {
			if(eventType.equalsIgnoreCase(EVENT_TYPE_LOAD)) {
				...
				logInfo("EVENT_TYPE_CLICK","Inside EVENT_TYPE_CLICK");
				if(TABCLICK.equalsIgnoreCase(controlName)) {
					onTabClickDeliveryMaker(data);
				} else if(BTN_SUBMIT.equalsIgnoreCase(controlName)){
					if(submitWorkitem(data)){
						return getReturnMessage(true,controlName);
					}
				} else if("GENERATETEMP".equalsIgnoreCase(controlName) || BUTTON_RETRY.equalsIgnoreCase(controlName)) {
				...
				} else if(BUTTON_REFRESH.equalsIgnoreCase(controlName)) {
					...
					int sOutput=updateDataInDB(sExtTable,"delivery_maker_name,DELIVERY_MAKER_DEC,WI_COMPLETED_FROM,"
							+ "DELIVERY_MAKER_SUBMIT_DATE","'"+sUserName+"'"+(char)25+"'"+formObject.getValue(CRO_DEC)+"'"+(char)25+"'"
	...
	public void onTabClickDeliveryMaker(String data){
	...
	private void onLoadDeliveryMaker() {
		logInfo("onLoadDeliveryMaker","Inside onLoadDeliveryMaker");
		prefLang();
		Frame_delivery();		
		hideControls(new String[]{NIG_MAKER,NIG_CHECKER,PIC1});
		showControls(new String[]{STAFF_ID,DELIVERY_DATE});
	...
	private boolean submitWorkitem(String data){
		if(formObject.getValue(CRO_DEC).toString().equalsIgnoreCase("Delivery is Successful")) {					
			if(!validateFutureDate(formObject.getValue(DELIVERY_DATE).toString(),DELIVERY_DATE)) {
				return false;
			} else if(checkMandatoryDoc(data)!= true) {
