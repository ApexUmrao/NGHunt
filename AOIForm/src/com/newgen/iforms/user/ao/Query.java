...
public class Query extends Common implements Constants, IFormServerEventHandler{

	public Query(IFormReference formObject) {
		super(formObject);
	}
	...
	public String executeServerEvent(IFormReference formObject, String controlName, String eventType, String data) {
		log.info("Inside executeServerEvent kdd >");
			...
			if(eventType.equalsIgnoreCase(EVENT_TYPE_LOAD)) {
				...
				if(TABCLICK.equalsIgnoreCase(controlName)) {
					...
					logInfo("Inside onClickEventCPDMakerFourStep controlName1: "
							,controlName+" eventType: "+eventType+" data: "+data);
					onClickEventQuery(controlName, eventType, data);
				}
	...
	public void onClickEventQuery(String controlName,String eventType,String data){
		if(controlName.equalsIgnoreCase("view")) {
	...
	public void onTabCPDMakerFourStep(String data) {
		...
		} else if(selectedSheetIndex == 5 || selectedSheetIndex == 6) {
			try { 
	...
	private void eventOnLoadQuery(String controlName,String  eventType,String data) {
		...
		try
		{
    		...
    		logInfo("Inside decision  eventType: ",formObject.getTableCellValue(ACC_RELATION, 0,1).toString());
    		logInfo("Inside decision  eventType: ",formObject.getTableCellValue(ACC_RELATION, 0,5).toString());
    		logInfo("Inside decision  eventType: ",formObject.getTableCellValue(ACC_RELATION, 0,2).toString());
		...
		if(!sActivity.equalsIgnoreCase("QUERY1"))
	{
		...
		else if(sActivityName.equalsIgnoreCase("DDE_CUST_INFO") || sActivityName.equalsIgnoreCase("CUSTOMER_SCREEN"))
		{
			setDDEModeCombos();
			...
			populateTRSD();
			if(sActivityName.equalsIgnoreCase("CUSTOMER_SCREEN"))
			{
				populateScreeningDataCRO();			
			}
		}
		else if(sActivityName.equalsIgnoreCase("QDE_CUST_INFO") || sActivityName.equalsIgnoreCase("CUSTOMER_SCREEN_QDE"))
		{
		...
		}		else if(sActivityName.equalsIgnoreCase("DDE_ACCOUNT_INFO") || sActivityName.equalsIgnoreCase("DDE_ACC_INFO_CHK"))
		{
			setDDEModeCombos();
			populatePersonalDataCPD();
			populateRiskData();
